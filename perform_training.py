...
#                        mask_gradients_all(dict(models_list[0].named_parameters()),gradient_masks)
    ...
    if 'train1' in phase or 'val' in phase or 'test' in phase:
        if classification is not None and classification != '2-way':
            ...
            #print(labels,probs)
            #try:
                #torch.save(labels,os.path.join('/home/scro3517/Desktop','labels'))
                #torch.save(probs,os.path.join('/home/scro3517/Desktop','probs'))
                #print('Hello')
            
            #if trial == 'abstention_penalty':
            #    epoch_auroc = roc_auc_score(labels,probs[:,0])
            #else:
            epoch_auroc = roc_auc_score(labels,probs)
            
            #epoch_auroc = roc_auc_score(labels.reshape(-1,1),probs.reshape(-1,1)) #use this for multilabel (not best option)
...
#            epoch_auroc = np.mean(all_auc)
            ...
            #except:
                #epoch_auroc = roc_auc_score(label)
             #   epoch_auroc = roc_auc_score(labels,np.reshape(probs,(-1,1)))
                #epoch_auroc = 0
    else:
        epoch_auroc = float('inf')
    
    """ Only for Covid Net """
#    binary_preds = np.where(probs>0,1,0)
#    #print(labels,binary_preds)
#    epoch_recall = recall_score(labels,binary_preds)
#    epoch_precision = precision_score(labels,binary_preds)
#    print('Recall: %.3f - Precision: %.3f' % (epoch_recall,epoch_precision))
...
def one_epoch(mixture,classification,criterion,criterion_single,weighted_sampling,phase,inference,dataloaders_list,models_list,mix_coefs,optimizer,device,bptt_steps=0,aul_scaling_dict=None,hyperparam_dict=None,epoch_count=None,new_task_epochs=None,trial=None,save_path_dir=None,lambda1=1,mask_gradients=False,reg_term=False,current_task_info=None,task_instance_params_dict=None):
